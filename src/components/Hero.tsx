...
const Hero = () => {
  ...
  const memoizedLogos = useMemo(() => <div className="absolute top-4 left-1/2 transform -translate-x-1/2 bg-white/10 backdrop-blur-sm p-2 rounded-lg z-10 flex items-center gap-4">
      <img src="/lovable-uploads/571af9aa-165e-4176-8c72-dbc808adab16.png" alt="Logo Ministerio de Educación y UGEL 03" className="h-14 w-auto object-contain" loading="eager" decoding="sync" />
      <img src="/lovable-uploads/4cb9c13e-f59b-4926-87f6-97248df9bec7.png" alt="Logo CETPRO PROMAE MAGDALENA" className="h-14 w-auto object-contain" loading="eager" decoding="sync" />
    </div>, []);
  return <div className="relative h-screen w-full overflow-hidden">
      {/* Background Images with optimized rendering */}
      {imageData.map((item, index) => <div key={index} className={cn("absolute inset-0 bg-cover bg-center transition-opacity duration-1000", currentImage === index ? "opacity-100" : "opacity-0")} style={{
      ...
      <div className="absolute bottom-16 left-1/2 transform -translate-x-1/2 flex space-x-2">
        {imageData.map((item, index) => <button key={index} className={cn("w-2 h-2 rounded-full transition-all duration-300", currentImage === index ? "bg-white w-6" : "bg-white/50")} onClick={() => handleImageSelect(index)} aria-label={`Ver imagen de ${item.title}`} />)}
      </div>
